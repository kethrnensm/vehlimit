...
public:
    ...
    public:
        bool init(address_t addr_in_module) {
            ...
            auto pe = reinterpret_cast<IMAGE_NT_HEADERS*>(static_cast<uintptr_t>(reinterpret_cast<uintptr_t>(dos) + dos->e_lfanew));
            if (pe->Signature != IMAGE_NT_SIGNATURE) {
                return false;
            }
            _base = reinterpret_cast<uintptr_t>(info.AllocationBase);
            ...
            _base = reinterpret_cast<uintptr_t>(info.dli_fbase);
            _size = buf.st_size;
#endif
            return true;
        }
        ...
        bool find(const char* pattern, const char* mask, address_t& addr) const {
            ...
            while (current_byte < last_byte) {
                for (i = 0; mask[i]; ++i) {
                    if ((current_byte + i) >= last_byte || ((mask[i] != '?') && (static_cast<byte_t>(pattern[i]) != current_byte[i]))) {
                        break;
                    }
                }
                if (!mask[i]) {
                    addr = reinterpret_cast<uintptr_t>(current_byte);
                    return true;
                }
                ++current_byte;
            }
    ...
    public:
        ...
        void enable() {
            if (_enabled) return;
            std::copy_n(reinterpret_cast<bytearray_t::value_type*>(_patch_addr), _new_data.size(), _original_data.data());
            std::copy_n(_new_data.data(), _new_data.size(), reinterpret_cast<bytearray_t::value_type*>(_patch_addr));
            _enabled = true;
        }

        void disable() {
            if (!_enabled) return;
            std::copy_n(_original_data.data(), _original_data.size(), reinterpret_cast<bytearray_t::value_type*>(_patch_addr));
            _enabled = false;
        }
...
private:
#ifdef _WIN32
    template<calling_convention> struct invoker;

    template<> struct invoker<calling_convention::cdeclcall> {
    ...
    template<> struct invoker<calling_convention::stdcall> {
        template<typename Ret, typename ... Args>
        static inline Ret call(address_t address, Args ... args) {
    ...
    template<> struct invoker<calling_convention::thiscall> {
        template<typename Ret, typename ... Args>
        static inline Ret call(address_t address, Args ... args) {
